# 📷 現場照片清冊自動套印工具 (Photo Report Generator) v2.3.0

[![Version](https://img.shields.io/badge/version-v2.3.0-blue.svg)](https://github.com/lianghao02/Photo-Report-Generator)
...
> **版本**：v2.3.0 (可復原工作台．PDF／Word 雙輸出．Tauri v2 桌面版)<br>
...
> 🔒 **100% 離線安全保證**：所有相依資源（樣式、圖示、Word/PDF/Excel/ZIP 生成核心）均已內建打包。無論是網頁版或桌面版，**圖片處理與檔案匯出均在您的本機記憶體內完成，絕不上傳任何伺服器或外部網路**，完全符合公務機敏資安標準。
...
v2.0/v2.1/v2.2 版本迎來重大架構革新：**徹底淘汰舊有 VBA 巨集與本機 Office 依賴**，升級為純前端單頁 Web 應用程式 (SPA) 與 Tauri 桌面版。使用者只需使用瀏覽器打開網頁或雙擊桌面版，即可批次拖曳照片、智慧填寫時間與說明，並直接在記憶體中生成符合公務標準的 **原生 Word (`.docx`) 與 PDF (`.pdf`) 清冊**，100% 零巨集資安警示！
...
### 步驟 3：一鍵匯出
- 在「版型與匯出」區選擇「清冊版型」（上下兩張／左右兩張／橫式三張），點擊 **「📄 匯出 Word 現場照片清冊」** 或 **「📕 匯出 PDF」** 即可於數秒內完成輸出！
...
├── index.html               # 核心應用程式 (v2.3.0 純前端 SPA 工作台)
...
└── scripts/
    ...
    ├── prepare-web.ps1      # 桌面版前端資源同步腳本 (自動整合 index.html、vendor/ 與 version.txt)
